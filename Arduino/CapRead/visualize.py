...
def show_values(pc, fmt="Diff = %.2f\n\nE%d = %.2f\n%s", **kw):
    ...
    for p, color, diff, E in zip(pc.get_paths(), pc.get_facecolors(), pc.get_array(), Elist):
        ...
        if(len(extra) == 12):
            ...
            extra[E-1] = extra[E-1][: extra[E-1].find(
                "-")] + 'uA' + extra[E-1][extra[E-1].find("-"): extra[E-1].find(")")] + 'uS'
            ax.text(x, y, fmt % (diff, E, float(val),
                    extra[E-1]), ha="center", va="center", color=color, **kw)
...
def heatmap(AUC):
    ...
    # Plot it out
#     if(firstTime):
    c = ax.pcolor(initial-AUC, edgecolors='k', linestyle='dashed',
                  linewidths=0.2, cmap='magma_r', vmin=0.0, vmax=511.0)
#     else:
#         c.set_data(AUC)
