...
uint8_t IT_config_ind = 0;
uint8_t IT_config[2][7] = {{16, 20, 24, 32, 36, 42, 63},
...
boolean Adafruit_MPR121_STM32::begin(uint8_t i2caddr) {
  ...
  writeRegister(MPR121_CONFIG1, 0x10); //16uA
  writeRegister(MPR121_CONFIG2, 0x40); // 0.5uS encoding, 1ms period
  //writeRegister(MPR121_CONFIG2, 0xB0); // 8uS encoding, 1ms period
...
void Adafruit_MPR121_STM32::reCalibrate(uint8_t regToRecal) {
  unsigned int ADC_count[12];
  bool isRecal = true;
  while (true)
  {

    for (int i = 0; i < regToRecal ; i++)
    {
      ADC_count[i] = this->filteredData(i);
      if (ADC_count[i] > 500)
      {
        isRecal = false;
        break;
    ...
    if (isRecal && IT_config_ind < 7)
    {
      IT_config_ind++;
      writeRegister(MPR121_ECR, 0x0);
      writeRegister(MPR121_CONFIG1, IT_config[0][IT_config_ind] & 0x3F); // ReCalibrating the Current Values
      writeRegister(MPR121_CONFIG2, IT_config[1][IT_config_ind] & 0xE0); // ReCalibrating the Time Values
      writeRegister(MPR121_ECR, 0x8F);  // start with first 5 bits of baseline tracking
      delay(200);
    }
    else
  ...
  Serial.println(IT_config_ind);
  Serial.println(IT_config[0][IT_config_ind] & 0x3F);
  Serial.println(IT_config[1][IT_config_ind] & 0xE0);
}
