...
float bodyArea()
{
  ...
  if ('~' == Serial.read()) //verify if string is starting with telda i.e no data loss from start
  {
    while(Serial1.available()) //get electrode values from capacitors 
    {
      readValue = Serial1.read();
      if (readValue == '~') //verify complete data has been transfered
      {break;}
      serialRead = serialRead + readValue;
    }
  }
  else
  {
